...
describe('auth state', () => {
  ...
  it('preserves whitelist on password reset and increments revision for every authority change', async () => {
    ...
    const captcha = await target.updateCaptchaMode('after-failures')
    expect(captcha.captchaMode).toBe('after-failures')
    expect(captcha.revision).toBe(whitelisted.revision)
    const reset = await target.reset('owner2', 'Another-Strong-43!')
    expect(reset.whitelist).toEqual(whitelisted.whitelist)
    expect(reset.captchaMode).toBe('after-failures')
