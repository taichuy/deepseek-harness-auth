...
function AccountSection() {
  ...
  return <section className="dsh-auth-section">
    ...
    {loadError ? <p className="dsh-auth-message" data-error="true">{zh ? '无法读取账号信息。' : 'Could not load account details.'}</p> : account === undefined ? <p className="dsh-auth-message">{zh ? '正在加载…' : 'Loading…'}</p> : <>
      ...
      {account.mode === 'whitelist' ? <p className="dsh-auth-message">{zh ? '当前请求通过 IP 白名单放行。为避免白名单访问直接修改凭据，请先使用账号登录后再重置密码。' : 'This request was allowed by the IP whitelist. Sign in with the account before changing credentials.'}</p> : <form className="dsh-auth-form" onSubmit={event => { void submit(event) }}>
